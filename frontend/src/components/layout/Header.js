  ...
  const navigationItems = [
    {
      ...
      dropdownItems: [
        {
          name: "Gallery",
          description: "Browse stunning community photos and artwork",
          href: "/feed",
          icon: <Compass className="w-5 h-5 text-blue-400" />,
        },
        {
          name: "Artists",
          description: "Discover talented creators worldwide",
          href: "/users",
          icon: <Users className="w-5 h-5 text-green-400" />,
        },
        {
          name: "Tags",
          description: "Find photos by themes and categories",
          href: "/tags",
    ...
    },
    {
      ...
      dropdownItems: [
        {
          name: "Dashboard",
          description: "Track your posts, likes, followers and analytics",
          href: "/dashboard",
        ...
        {
          name: "Collections",
          description: "Organise and curate your favourite images",
          href: "/collections",
                        ...
                        >
                          <div className="p-2">
                            {item.dropdownItems.map((dropdownItem, idx) => (
                              ...
                              >
                                <div
                                  className="flex items-start p-4 rounded-xl transition-all duration-200 relative overflow-hidden cursor-pointer hover:bg-white/10"
                                  onClick={() => {
                                    if (dropdownItem.href) navigateTo(dropdownItem.href);
                                  }}
                                >
                                  <motion.div
                                  ...
                                  <div className="ml-4 flex-1 relative z-10">
                                    <div className="flex items-center justify-between">
                                      ...
                                      <div className="flex items-center space-x-2">
                                        {dropdownItem.badge && (
                                          <span className="px-2 py-1 text-xs font-bold text-white rounded-full bg-gradient-to-r from-emerald-500 to-cyan-500">
                                            {dropdownItem.badge}
                                          </span>
                                        )}
              ...
              {user ? (
                <>
                  ...
                  <motion.button
                    ...
                    className="hidden sm:flex items-center space-x-1.5 px-3.5 py-2 text-sm font-medium text-white bg-gradient-to-r from-violet-600 to-fuchsia-600 rounded-xl shadow-md hover:shadow-violet-500/25 transition-all"
                  >
                    <span>Feed</span>
                  ...
                  <div className="relative">
                    <motion.button
                      whileHover={{ scale: 1.05 }}
                      whileTap={{ scale: 0.95 }}
                      onClick={() => navigateTo(user.username ? `/profile/${user.username}` : '/dashboard')}
                      className="flex items-center space-x-2 p-1 rounded-xl hover:bg-white/10 transition-all duration-200 ring-1 ring-white/10"
                      title={user.fullName || user.username || "My Profile"}
                    >
                      <div className="w-8 h-8 rounded-lg overflow-hidden bg-gradient-to-br from-violet-500 to-fuchsia-500 flex items-center justify-center text-white font-bold text-sm">
                        {user.profilePicture && !user.profilePicture.includes('default-profile') ? (
                          <img src={user.profilePicture} alt={user.username || "User"} className="w-full h-full object-cover" />
                        ) : (
                          <span>{(user.username || user.fullName || "U").charAt(0).toUpperCase()}</span>
                        )}
                      </div>
                    </motion.button>
                  </div>
                ...
                <>
                  {/* Visitor Authentication Buttons */}
                ...
                {user ? (
                  <div className="space-y-3">
                    <div className="flex items-center space-x-3 p-3 rounded-xl bg-white/5 border border-white/10">
                      <div className="w-10 h-10 rounded-xl overflow-hidden bg-gradient-to-br from-violet-500 to-fuchsia-500 flex items-center justify-center text-white font-bold">
                        {user.profilePicture && !user.profilePicture.includes('default-profile') ? (
                          <img src={user.profilePicture} alt={user.username || "User"} className="w-full h-full object-cover" />
                        ) : (
                          <span>{(user.username || user.fullName || "U").charAt(0).toUpperCase()}</span>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-gray-400 text-xs truncate">@{user.username || "creator"}</p>
                      </div>
                    </div>
                    <motion.button
                      whileHover={{ scale: 1.02 }}
                      whileTap={{ scale: 0.98 }}
                      className="w-full p-3.5 rounded-xl bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white font-semibold flex items-center justify-center transition-colors shadow-lg"
                      onClick={() => navigateTo("/feed")}
                    >
                  ...
                  <div className="space-y-3">
                    <motion.button
                      whileHover={{ scale: 1.02 }}
                      whileTap={{ scale: 0.98 }}
                      className="w-full p-4 rounded-xl text-white bg-white/10 hover:bg-white/20 font-medium flex items-center justify-center transition-colors border border-white/10"
                      onClick={() => navigateTo("/login")}
                    >
                    ...
                    <motion.button
                      whileHover={{ scale: 1.02 }}
                      whileTap={{ scale: 0.98 }}
                      className="w-full p-4 rounded-xl bg-gradient-to-r from-violet-600 to-fuchsia-600 hover:from-violet-500 hover:to-fuchsia-500 text-white font-semibold flex items-center justify-center transition-colors shadow-lg shadow-violet-500/25"
                      onClick={() => navigateTo("/register")}
                    >
